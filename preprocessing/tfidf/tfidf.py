...
class TFIDF:
    ...
    def fit(self, dataset):
        ...
        word_doc_count = {}
        for doc_words in dataset:
            unique_words = set(doc_words.split())
            for word in unique_words:
                word_doc_count[word] = word_doc_count.get(word, 0) + 1

        self.idf_dict = {
            word: np.log(total_docs / (count + 1)) + 1
    ...
    def transform(self, dataset):
        tfidf_matrix = np.zeros((len(dataset), len(self.vocabulary)))

        for i, doc in enumerate(dataset):
          word = doc.split()
          word_counts = pd.Series(word).value_counts()
          total_words = len(word)

          for j, vocab_word in enumerate(self.vocabulary):
            tf = word_counts.get(vocab_word, 0) / total_words
            idf = self.idf_dict[vocab_word]
